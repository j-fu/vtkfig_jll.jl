...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import vtkfig_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("vtkfig_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "vtkfig_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"vtkfig", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "vtkfig_jll", "src", "vtkfig_jll.jl"))
        @info("vtkfig_ll dev'ed out to /home/fuhrmann/Wias/work/julia/dev/vtkfig_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("10287013-a584-5a38-a29a-42553acd0316"))
platforms = [Pkg.Artifacts.unpack_platform(e, "vtkfig", artifacts_toml) for e in artifacts["vtkfig"]]
...
if best_platform === nothing
    @debug("Unable to load vtkfig; unsupported platform $(triplet(platform_key_abi()))")
else
